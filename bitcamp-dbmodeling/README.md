...
- 후보키(candidate key) 선정
  - key 들 중에서 중복되는 칼럼을 제거한 후 최소키를 선정한다.
...
- 자식 테이블 쪽에 참조하는 부모 체이블의 PK 컬럼을 놓는다.
- 자식테이블에 놓은 부모 테이블의 PK 값을 '외부키(foreign key)' 라 부른다.
- 각 데이터가 다른 테이블의 데이터 몇 개와 관계를 맺는지 차수를 정한다.
...
- 여러 테이블에 공통으로 포함되는 컬럼이 있는 경우를 별도의 테이블로 분리하여 정의한다.
  다음 중 하나의 관계로 설정한다.
